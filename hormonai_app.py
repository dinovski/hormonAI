...
# Let user choose language, then derive all labels from it
...
Discutez toujours de votre situation et de toute décision thérapeutique directement avec votre équipe d’oncologie.
    ...
    if "prompt_label_used" not in st.session_state:
        st.session_state.prompt_label_used = "fr"
    placeholder = "Par exemple : « Quels effets secondaires sont fréquents avec le tamoxifène ? »"
    ...
    use_llm_help = (
        ...
        "Si activé, le LLM reformule à partir du contexte de la FAQ, avec des garde-fous stricts."
    )
            ...
            if show_sources and msg.get("sources"):
                ...
                with st.expander(exp_label):
                    for i, src in enumerate(msg["sources"], start=1):
                        st.markdown(
                            f"**Source {i}** – relevance: `{src['score']:.3f}`"
                        )
...
def handle_send(
    ...
    st.session_state.history.append(
        {"role": "user", "content": user_input_val, "sources": None}
    )

    # Retrieve debug results for source display
    try:
        debug_results = retriever_obj.retrieve(user_input_val, top_k=3)
    except Exception:
        debug_results = []
...
# ---------- USER INPUT ----------
st.markdown(
    f'<div class="prompt-label">{prompt_label}</div>',
    unsafe_allow_html=True,
)
