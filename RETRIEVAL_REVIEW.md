...
- **Article representation on the topic channel:** article chunks have no question, and PDFs have no headings, so the "topic" (Q) and "topic+paraphrases" (Qp) dense channels were embedding only the heading breadcrumb — for PDFs, just the file name (100% of 274 article chunks were `"Section: <filename>"`, i.e. two of the four channels were noise for articles). `q_text_for` now uses the heading plus the chunk's leading text as a topic proxy, so all four channels carry article content. **Requires a re-ingest** to rebuild the topic/paraphrase FAISS indexes.
...
### Round 7 (2026-07-30) — latency, answer presentation, compassion guardrails, language scope

Operational and safety refinements; no change to the core gate.

- **Reranker latency (main lever):** `retrieve()` now reranks only a shortlist of the top fused candidates instead of the whole `top_k` pool (`rerank_top_n`, default 20). The cross-encoder is linear in the number of pairs, so with a large reranker (e.g. `bge-reranker-v2-m3`) over ~40 candidates this was the dominant per-query cost. Tunable via `HORMONAI_RERANK_TOP_N` / `--rerank-top-n` across CLI, GUI, and eval.
- **Timing instrumentation:** per-query retrieve / rerank / llm / total is captured on `AnswerResult.timing_ms` (always populated), printed by the CLI `--debug` line and an opt-in GUI caption (`HORMONAI_SHOW_TIMING`). PHI-free latency records (timing + language + answered only, never the query) can be logged with `HORMONAI_AUDIT_LATENCY`; `tools/latency_report.py` aggregates them into p50/p95/p99 per stage.
- **Source presentation:** citations are no longer appended to the answer body. They are returned on `AnswerResult.sources_text` and shown in the GUI "Sources used for this answer" panel (built from `source_indices`) or printed after the CLI answer. Keeps the patient-facing reply clean while preserving verifiability.
- **Compassion guardrails (deterministic, applied to the final answer, both verbatim and LLM paths):** `_reframe_survival` rewrites patient-mortality phrasing ("dying from breast cancer") into survival language, narrowly scoped so accurate cancer-cell-death statements are preserved; `_strip_llm_preamble` removes a leading meta-preamble ("Here is a clear and compassionate response:"). Both hold regardless of what the local LLM emits; the rephrase prompts were also hardened.
- **GUI launch passthrough:** the app reuses the CLI argument parser, so `streamlit run hormonai_app.py -- <same flags>` configures the GUI; precedence is flag > env > default.
- **Language scope option:** `--retrieval-scope {shared,language}` (env `HORMONAI_RETRIEVAL_SCOPE`) selects the combined index with cross-lingual fallback (`shared`, default) or the query-language corpus only (`language`, using the per-language `kb_<lang>` index with no fallback). No core retrieval change: `language` reuses the existing per-language path, where the cross set is already empty. The GUI defaults to `shared` to preserve current behavior and falls back to the shared index if a per-language index is missing.
