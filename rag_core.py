...
EN_KEEP = {"pregnancy","recurrence","tamoxifen","aromatase","inhibitor","clot","thrombosis","fertility","child","pause"}
...
def extract_keywords(query: str, language: str) -> List[str]:
    ...
    if language == "en":
        ...
        for t in toks:
            if t in EN_KEEP:
                kws.append(t)
            elif t not in EN_STOPWORDS and len(t) >= 4:
                kws.append(t)
        seen = set()
        out = []
        for k in kws:
        ...
        return out[:12]
    kws = [t for t in toks if t not in FR_STOPWORDS and len(t) >= 4]
    seen = set()
    out = []
    for k in kws:
        if k not in seen:
...
def choose_best_candidate(
    ...
    min_overlap = 1
    # If user clearly asks about pregnancy, insist pregnancy appears somewhere in the candidate
    if language == "en" and "pregnancy" in kws:
        min_overlap = 1

    scored = [(overlap_score(kws, c), c) for c in candidates]
    passed = [c for ov, c in scored if ov >= min_overlap]

    if debug:
        print("\n[DEBUG] keywords:", kws)
        for ov, c in sorted(scored, key=lambda x: (-x[0], -x[1].get("fused_score", 0)))[:10]:
            print(f"  overlap={ov} idx={c['index']} fused={c.get('fused_score', 0):.5f} | Q={c['question'][:80]}")
        print(f"  passed(min_overlap={min_overlap})={len(passed)}")

    if not passed:
        return None

    # Prefer rerank_score if present
...
class HybridFAQRetriever:
    def __init__(
        ...
        self.index_q = faiss.read_index(f"{prefix}_index_q.faiss")
        self.index_qa = faiss.read_index(f"{prefix}_index_qa.faiss")

        with open(f"{prefix}_qa.pkl", "rb") as f:
            qa = pickle.load(f)
        self.items = qa["items"]
